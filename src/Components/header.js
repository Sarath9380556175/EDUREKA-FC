...
{
    axios({
        url:'https://young-depths-27513.herokuapp.com/signups',
        method:'get',
        headers:{'content-Type':'application/json'},

    })
    .then(res=>this.setState({mails:res.data.signups.map((item)=>{return item.email})}))
...
createaccount =(state,value)=>{
    ...
    { 
        ...
        const {firstname,lastname,password,email,gender,mails}=this.state;
...
console.log(responses);
 ...
 axios({
     url:'https://young-depths-27513.herokuapp.com/login',
     method:'POST',
     headers:{'content-Type':'application/json'},
     data:{
 email:emailid,
